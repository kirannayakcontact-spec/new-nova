...
// Admin UI/API for KALYAN WhatsApp role targets.
// This module mutates the live arrays exported by roleRouter.js, so saved UI changes
// apply to the running Gateway without changing Manual Overwrite Mode.
...
function firebaseChildUrl(childPath) {
  return firebaseDataUrl().replace(/\.json(?:[?#].*)?$/, `/${childPath}.json`);
}
...
async function readRoleConfig() {
  const res = await axios.get(firebaseChildUrl("marketRoleTargets/KALYAN"), { timeout: 8000 });
  return res.data && typeof res.data === "object" ? res.data : {};
}
async function saveRoleConfig(config) {
  const res = await axios.put(firebaseChildUrl("marketRoleTargets/KALYAN"), config || {}, { timeout: 10000 });
...
async function refreshLiveTargets() {
  try {
    ...
    console.log("[roleRouterUi] KALYAN UI role targets loaded:", countsFor(targets));
...
function roleRouterPage() {
  ...
  <div class="card"><label style="font-weight:900"><input id="enabled" type="checkbox" style="width:auto;margin-right:8px"> Enable Role Router</label><p class="muted">OFF karne par UI targets ignore honge aur Gateway ka normal behaviour chalega.</p></div>
  ${[
    ["entryTargets","Entry Target 🎮","KALYAN_ADMIN_GROUP: VIP entries yahan accept hongi"],
    ["bookieTargets","Bookie Target 🛡️","KALYAN_ADMIN_GROUP: withdrawal, payment, admin alerts"],
    ["scheduleTargets","Schedule Target 📅","KALYAN_INTEL_GROUP: Daily Intel only"],
    ["resultTargets","Result Target 🏆","KALYAN_RESULT_GROUP: Open/Close result only"],
    ["forwardTargets","Forward Target 📊","BOOKIE_LOAD_REPORT_GROUP: Load Report only"]
  ...
  <div class="card"><button onclick="save()">Save Role Targets</button><button class="danger" style="margin-top:8px" onclick="resetEnv()">Clear UI Targets / Use .env</button><button class="secondary" style="margin-top:8px" onclick="load()">Refresh</button></div>
...
async function load(){const r=await fetch('/api/role_router');const d=await r.json();if(d.status!=='success'){document.getElementById('status').textContent=d.message||'Load failed';return}const c=d.config||{};document.getElementById('enabled').checked=c.enabled!==false;['entryTargets','bookieTargets','scheduleTargets','resultTargets','forwardTargets'].forEach(k=>setVal(k,c[k]||[]));document.getElementById('status').innerHTML='Saved UI targets loaded • Live counts: '+JSON.stringify(d.runtimeCounts||{});document.getElementById('enabled-pill').textContent=(c.enabled!==false?'ON':'OFF');document.getElementById('enabled-pill').className='pill '+(c.enabled!==false?'ok':'danger');document.getElementById('groups').innerHTML=(d.groups||[]).map(g=>'<div style="padding:8px 0;border-bottom:1px solid rgba(255,255,255,.06)"><b>'+g.name+'</b><div class="muted">'+g.id+'</div></div>').join('')||'No cached groups yet. Connect WhatsApp once.'}
    ...
    if (app && typeof app.get === "function" && typeof app.post === "function" && !app.__titanRoleRouterUiInstalled) {
      ...
      app.get("/api/role_router", async (req, res) => {
        try {
          ...
          res.json({ status: "success", config: configForResponse(config), envFallback: ENV_FALLBACK, runtimeCounts: countsFor(targets), groups });
        } catch (error) { res.status(500).json({ status: "error", message: error.message || String(error), config: {}, groups: [] }); }
      });
      app.post("/api/role_router", async (req, res) => {
        try {
          const body = req.body || {};
          ...
          res.json({ status: "success", config: configForResponse(config), runtimeCounts: countsFor(targetsFromConfig(config)) });
...
module.exports = { applyLiveTargets, refreshLiveTargets, targetsFromConfig, sanitizeConfig };
